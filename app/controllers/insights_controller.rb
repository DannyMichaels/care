class InsightsController < ApplicationController
  ...
  def index
    ...
    render json: @insights.map {|insight| insight.attributes.except('updated_at').merge( {user: insight.user.attributes.except('password_digest', 'created_at', 'email', 'updated_at', 'birthday'), likes: insight.likes.map {|like| like.attributes.except('updated_at')} })}
  end

  # GET /insights/1
  def show
    render json: @insight.attributes.except('updated_at').merge( {user: @insight.user.attributes.except('password_digest', 'updated_at', 'email')})
  end
